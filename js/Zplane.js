...
class Zplane {
    ...
    //add a conj to zero or a pole according to type omar
    add_conjugtes=(type)=>{
        let x=this.dataSet.length
        console.log("Gowa Zplane addConj foo2 el for:",this.dataSet.length)
        for (let i = 0; i < x; i++) {
            console.log("Gowa Zplane addConj bara el if:",this.dataSet[i].type)
            if ((this.dataSet[i].type == type)) {
                console.log("Gowa Zplane addConj fe el if:",this.dataSet[i].type)
                this.add_point([this.dataSet[i].point[0], -1 * this.dataSet[i].point[1]],"nonConjPole");
                // this.dataSet.push({
                //     point: [this.dataSet[i].point, -1 * this.dataSet[i].point],
                //     type: type
                // });
                this.dataSet.splice(i, 1);
                this.dataSet[i].point.remove()

            }
        }
        console.log("Gowa Zplane addConj ta7t el for:",this.dataSet.length)
